...
function normalizar(texto) {
  return texto
    ...
    .replace(/\.(?!\d)/g, ' ')
    .replace(/[!?;:]/g, ' ')
    .replace(/\bmil\s+e\s+quinhentos\b/g, '1500')
    .replace(/\bmil\s+e\s+cem\b/g, '1100')
    .replace(/\bmil\s+e\s+duzentos\b/g, '1200')
    .replace(/\bmil\s+e\s+trezentos\b/g, '1300')
    .replace(/\bmil\s+e\s+quatrocentos\b/g, '1400')
    .replace(/\bmil\s+e\s+seiscentos\b/g, '1600')
    .replace(/\bmil\s+e\s+setecentos\b/g, '1700')
    .replace(/\bmil\s+e\s+oitocentos\b/g, '1800')
    .replace(/\bmil\s+e\s+novecentos\b/g, '1900')
    .replace(/\bdez\s+mil\b/g, '10000')
    .replace(/\bnove\s+mil\b/g, '9000')
    .replace(/\boito\s+mil\b/g, '8000')
    .replace(/\bsete\s+mil\b/g, '7000')
    .replace(/\bseis\s+mil\b/g, '6000')
    .replace(/\bcinco\s+mil\b/g, '5000')
    .replace(/\bquatro\s+mil\b/g, '4000')
    .replace(/\btres\s+mil\b/g, '3000')
    .replace(/\bdois\s+mil\b/g, '2000')
    .replace(/\bmil\b/g, '1000')
    .replace(/\bquinhentos\b/g, '500')
    .replace(/\bquatrocentos\b/g, '400')
...
function extrairIdCartao(texto) {
  const explicito = texto.match(/(?:#|id\s+|n\s*|numero\s+|num\s+|hashtag\s+)(\d+)\b/);
...
function extrairNomeCartaoParaComando(texto) {
  ...
  const semAcoes = base
    ...
    .replace(/(?:#|id\s*|n\s*|numero\s*|num\s*|hashtag\s*)?\d+[.,]?\d*/g, '')
    .replace(/\b(numero|num|hashtag|n)\b/g, '')
    .replace(/\b(cartao|credito)\b/g, '')
    .replace(/\s+/g, ' ')
    .trim();

  return semAcoes || null;
}
